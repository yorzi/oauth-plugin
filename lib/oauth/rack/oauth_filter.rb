    ...
    class OAuthFilter
      ...
      def call(env)
        ...
        if token_string = oauth2_token(request)
          if token = Oauth2Token.where('invalidated_at IS NULL and authorized_at IS NOT NULL and token = ?',
                                       token_string).first
          ...
          if client_application
            ...
            if request_proxy.token
              oauth_token = client_application.tokens.where(
                'invalidated_at IS NULL AND authorized_at IS NOT NULL and token = ? AND (expires_at IS NULL OR expires_at >= ?)', request_proxy.token, Time.zone.now
              ).first
          ...
          env['oauth.version'] = 1
        elsif env['HTTP_AUTHORIZATION'].blank?
          # anonymous requests supports
          strategies << :two_legged
        end
