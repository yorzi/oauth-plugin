        ...
        if token_string = oauth2_token(request)
          ...
          if client_application
            ...
            if request_proxy.token
              oauth_token = client_application.tokens.where('invalidated_at IS NULL AND authorized_at IS NOT NULL and token = ? AND (expires_at IS NULL OR expires_at >= ?)', request_proxy.token, Time.zone.now).first
